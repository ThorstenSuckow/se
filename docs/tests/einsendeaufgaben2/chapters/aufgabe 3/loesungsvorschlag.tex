...
Es liegt also nahe, \code{Film} als \textit{Abstraction} zu verstehen, in dem gemeinsame Daten gespeichert werden, und \code{Vorstellung} als \textit{Occurence}, also das \textit{Auftreten} einer solchen Filmvorführung im gegebenen fachlichen Kontext ``Kino``.\\
...
\noindent
\code{Film} darf also nicht als Auftreten (\textit{Occurrence}) einer \textit{Abstraction} ``\code{Regissuer}`` verstanden werden.\\
