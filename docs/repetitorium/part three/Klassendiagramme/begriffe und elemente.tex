...
Die \textbf{Übergaberichtung} gibt ab, wie ein Parameter verarbeitet wird.
Mögliche Werte sind
...
\noindent
Dabei ist die \textbf{Generalisierung} ein zentrales Konzept des objektorientierten Designs - der grundlegende Gedanke hier ist, dass alle \textit{vererbbaren} Features eines \textbf{Supertyps} auf den \textbf{Subtyp} übergehen (\textbf{Vererbung}).\\
...
Das Substitutionsprinzip gilt nur für die Implementierungsvererbung und hier nur für die konkrete Klasse (vgl.~\cite[26]{Buh09}).\\


\begin{tcolorbox}[title=Subtyping und Subclassing,colback=red!20]
    tbd.
\end{tcolorbox}
